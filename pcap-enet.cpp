 ...
 *
 * Based on the MERIT NNstat etherifrt.cpp and the Ultrix pcap-pf.cpp
 * subroutines.
 *
...
void readloop(int cnt, int if_fd, struct bpf_program *fp, printfunc printit) {
  ...
  while (1) {
    ...
    while (cc > 0) {
      ...
      caplen = ph->tap.th_wirelen > snaplen ? snaplen : ph->tap.th_wirelen;
      if (pcap_filter(fcode, (char *)ph->packet, ph->tap.th_wirelen, caplen)) {
        if (cnt >= 0 && --cnt < 0)
          goto out;
        (*printit)((char *)ph->packet, (struct timeval *)ph->tap.th_timestamp,
...
static void efReadError(int fid, char *msg) {
...
void wrapup(int fd) {
  ...
  if (es.enStat_MaxRead > 1)
    fprintf(stderr, ", %d packets in largest read", es.enStat_MaxRead);
  putc('\n', stderr);
#endif /* IBMRTPC */
  close(fd);
}
...
int initdevice(char *device, int pflag, int *linktype) {
  ...
  if (if_fd == -1) {
    ...
    error("your system may not be properly configured; see \"man enet(4)\"");
    exit(-1);
  }
