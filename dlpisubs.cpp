/*
 * This code is derived from code formerly in pcap-dlpi.cpp, originally
 ...
 */
int pcap_process_pkts(pcap_t *p, pcap_handler callback, u_char *user, int count,
                      u_char *bufp, int len) {
  struct pcap_dlpi *pd = p->priv;
...
#else
    (void)gettimeofday(&pkthdr.ts, nullptr);
#endif
      pkthdr.len = origlen;
...
int pcap_process_mactype(pcap_t *p, u_int mactype) {
  ...
  default:
    snprintf(p->errbuf, PCAP_ERRBUF_SIZE, "unknown mactype 0x%x", mactype);
    retv = -1;
  }
 ...
 */
int pcap_conf_bufmod(pcap_t *p, int snaplen) {
  struct timeval to;
  ...
  ss = snaplen;
  if (ss > 0 && strioctl(p->fd, SBIOCSSNAP, sizeof(ss), (char *)&ss) != 0) {
 ...
 */
static void pcap_stream_err(const char *func, int err, char *errbuf) {
