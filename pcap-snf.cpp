...
static int snf_pcap_stats(pcap_t *p, struct pcap_stat *ps) {
  ...
  if ((rc = snf_ring_getstats(snfps->snf_ring, &stats))) {
    pcap_fmt_errmsg_for_errno(p->errbuf, PCAP_ERRBUF_SIZE, rc, "snf_get_stats");
    return -1;
  }
  ...
  while (n < cnt || PACKET_COUNT_IS_UNLIMITED(cnt)) {
    ...
    if (err) {
      if (err == EBUSY || err == EAGAIN) {
        ...
        pcap_fmt_errmsg_for_errno(p->errbuf, PCAP_ERRBUF_SIZE, err, "snf_read");
        return -1;
      }
    }
...
static int snf_inject(pcap_t *p, [[maybe_unused]] const void *buf,
  ...
  struct pcap_snf *ps = p->priv;
  int rc;
  if (ps->snf_inj == nullptr) {
    rc = snf_inject_open(ps->snf_boardnum, 0, &ps->snf_inj);
    if (rc) {
      pcap_fmt_errmsg_for_errno(p->errbuf, PCAP_ERRBUF_SIZE, rc,
                                "snf_inject_open");
      return (-1);
    }
  }
...
static int snf_activate(pcap_t *p) {
   ...
   */
  err = snf_open(
      ps->snf_boardnum, 0, /* let SNF API parse SNF_NUM_RINGS, if set */
      nullptr,             /* default RSS, or use SNF_RSS_FLAGS env */
      (p->opt.buffer_size > 0 && p->opt.buffer_size < 1048576)
          ? 1048576
          : p->opt.buffer_size, /* default to SNF_DATARING_SIZE from env */
      flags,                    /* may want pshared */
      &ps->snf_handle);
  if (err != 0) {
    pcap_fmt_errmsg_for_errno(p->errbuf, PCAP_ERRBUF_SIZE, err,
                              "snf_open failed");
    return -1;
  }

  if ((nr = getenv("SNF_PCAP_RING_ID")) && *nr) {
    ring_id = (int)strtol(nr, nullptr, 0);
  }
...
int snf_findalldevs(Interfaces *devlistp, char *errbuf) {
  ...
  if (snf_init(SNF_VERSION_API)) {
    (void)snprintf(errbuf, PCAP_ERRBUF_SIZE, "snf_getifaddrs: snf_init failed");
    return (-1);
  }

  if (snf_getifaddrs(&ifaddrs) || ifaddrs == nullptr) {
  ...
  for (ifa = ifaddrs; ifa != nullptr; ifa = ifa->snf_ifa_next) {
    /*
    ...
    if (dev != nullptr) {
       ...
       */
      dev = add_dev(devlistp, ifa->snf_ifa_name, 0, desc, errbuf);
      if (dev == nullptr)
        return -1;
#ifdef _WIN32
      /*
      ...
      if (ret == 1) {
        ...
        if (add_addr_to_dev(dev, &addr, sizeof(addr), nullptr, 0, nullptr, 0,
                            nullptr, 0, errbuf) == -1)
          return -1;
      } else if (ret == -1) {
        /*
  ...
  if (merge) {
    ...
    (void)snprintf(desc, MAX_DESC_LENGTH,
                   "Myricom Merge Bitmask All Ports snf%d", allports);
    /*
     ...
     */
    if (add_dev(devlistp, name, PCAP_IF_CONNECTION_STATUS_NOT_APPLICABLE, desc,
                errbuf) == nullptr)
      return (-1);
    /*
