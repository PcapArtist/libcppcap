...
LOCAL __inline BOOL PktInterrupt(void) {
  ...
  if (okay)
    pktInfo.error = nullptr;
  else
    pktInfo.error = PktGetErrorStr(reg.r_dx >> 8);
  return (okay);
}
...
#elif (DOSX & DOS4GW)
  memcpy(&pktInfo.name, (void *)DOS_ADDR(reg.r_ds, reg.r_si),
         sizeof(pktInfo.name));
...
PUBLIC int PktReceive(BYTE *buf, int max) {
  ...
  if (ofs != _farpeekw(_dos_ds, realBase + rxInOfs)) {
    ...
    if (ofs > LAST_RX_BUF)
      ...
      _farpokew(_dos_ds, realBase + rxOutOfs, ofs);
    return (size);
  }
  return (0);
}
...
PUBLIC DWORD PktRxDropped(void) { return *(DWORD *)(realBase + pktDrop); }
...
PUBLIC DWORD PktRxDropped(void) { return (pktDrop); }
#endif
...
PUBLIC BOOL PktExitDriver(void) {
  ...
  if (pcap_pkt_debug >= 1)
    printf("Internal stats: too-small %lu, too-large %lu, bad-sync %lu, "
           "wrong-handle %lu\n",
           intStat.tooSmall, intStat.tooLarge, intStat.badSync,
           intStat.wrongHandle);
  return (TRUE);
}
...
#if (DOSX & PHARLAP)
  if (RealCopy((ULONG)&rxOutOfs, (ULONG)&pktRxEnd, &realBase, &protBase,
               (USHORT *)&realSeg)) {
  ...
  *(WORD *)(realBase + rxInOfs) = FIRST_RX_BUF;
  ...
  if (writeInfo)
    printf("Pkt-driver information:\n"
           ...
           pktInfo.majVer, pktInfo.minVer, pktInfo.name, pktInfo.class,
           PktGetClassName(pktInfo.class), pktInfo.type, pktInfo.number,
           pktInfo.funcs, pktInfo.intr, pktInfo.handle,
           pktInfo.funcs == 2 || pktInfo.funcs == 6 ? "Yes" : "No",
           pktInfo.funcs == 5 || pktInfo.funcs == 6 ? "Yes" : "No",
           PktRXmodeStr(rxMode), myAddress[0], myAddress[1], myAddress[2],
 ...
 *     1      if success
 */
int RealCopy(ULONG start_offs, ULONG end_offs, REALPTR *real_basep,
