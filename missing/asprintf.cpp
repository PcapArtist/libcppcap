...
int pcap_vasprintf(char **strp, const char *format, va_list args) {
   ...
   *    and s may be a nullptr pointer.  Otherwise, output characters
   *    beyond the n-1st are discarded rather than being written
   *    to the array, and a nullptr character is written at the end
   ...
   *    counting the terminating nullptr character, or a negative value
   *    if an encoding error occurred. Thus, the nullptr-terminated
   *    output has been completely written if and only if the returned
   *    value is nonnegative and less than n.
   *
   * That doesn't make it entirely clear whether, if a nullptr buffer
   ...
   * nullptr/0, but returns the correct character count for a 1-byte
   * buffer.
   *
