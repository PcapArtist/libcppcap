/*
 * pcap-dag.cpp: Packet capture interface for Endace DAG cards.
 *
...
static void delete_pcap_dag(pcap_t *p) {
  ...
  for (prev = nullptr, curr = pcap_dags; curr != nullptr && curr->p != p;
...
static int new_pcap_dag(pcap_t *p) {
  pcap_dag_node_t *node = nullptr;

  if ((node = malloc(sizeof(pcap_dag_node_t))) == nullptr) {
    return -1;
  }
...
static int dag_read(pcap_t *p, int cnt, pcap_handler callback, u_char *user) {
  ...
  while (pd->dag_mem_top - pd->dag_mem_bottom < dag_record_size) {
     ...
     */
    if (nullptr == (pd->dag_mem_top = dag_advance_stream(
                        p->fd, pd->dag_stream, &(pd->dag_mem_bottom)))) {
      return -1;
    }

    if (nonblocking &&
        (pd->dag_mem_top - pd->dag_mem_bottom < dag_record_size)) {
      /* Pcap is configured to process only available packets, and there aren't
       * any, return immediately. */
      return 0;
    }

    if (!nonblocking && pd->dag_timeout &&
        (pd->dag_mem_top - pd->dag_mem_bottom < dag_record_size)) {
      /* Blocking mode, but timeout set and no data has arrived, return
       * anyway.*/
      return 0;
    }
  }
  ...
  while (pd->dag_mem_top - pd->dag_mem_bottom >= dag_record_size) {
    ...
    if (rlen < dag_record_size) {
      pcap_strlcpy(p->errbuf, "dag_read: record too small", PCAP_ERRBUF_SIZE);
      return -1;
    }
    ...
    /* ERF encapsulation */
    /* The Extensible Record Format is not dropped for this kind of
     * encapsulation, and will be handled as a pseudo header by the decoding
     * application. The information carried in the ERF header and in the
     * optional subheader (if present) could be merged with the libpcap
     * information, to offer a better decoding. The packet length is o the
     * length of the packet on the link (header->wlen), o plus the length of the
     * ERF header (dag_record_size), as the length of the pseudo header will be
     * adjusted during the decoding, o plus the length of the optional subheader
     * (if present).
     *
     * The capture length is header.rlen and the byte stuffing for alignment
     * will be dropped if the capture length is greater than the packet length.
     */
    if (p->linktype == DLT_ERF) {
      ...
      case ERF_TYPE_MC_AAL5:
        ...
        if (p->linktype == DLT_SUNATM) {
          ...
          sunatm->flags = ((header->flags.iface & 1) ? 0x80 : 0x00) |
                          ((sunatm->vpi == 0 && sunatm->vci == htons(5))
                               ? 6
                               : ((sunatm->vpi == 0 && sunatm->vci == htons(16))
                                      ? 5
                                      : ((dp[ATM_HDR_SIZE] == 0xaa &&
                                          dp[ATM_HDR_SIZE + 1] == 0xaa &&
                                          dp[ATM_HDR_SIZE + 2] == 0x03)
                                             ? 2
                                             : 1)));
      ...
      case ERF_TYPE_HDLC_POS:
        if ((p->linktype != DLT_CHDLC) && (p->linktype != DLT_PPP_SERIAL) &&
      ...
      case ERF_TYPE_MC_HDLC:
        if ((p->linktype != DLT_CHDLC) && (p->linktype != DLT_PPP_SERIAL) &&
            (p->linktype != DLT_FRELAY) && (p->linktype != DLT_MTP2) &&
            (p->linktype != DLT_MTP2_WITH_PHDR) && (p->linktype != DLT_LAPD))
          continue;
        packet_len = ntohs(header->wlen);
        packet_len -= (pd->dag_fcs_bits >> 3);
        caplen = rlen - dag_record_size - 4;
        ...
        if (p->linktype == DLT_MTP2_WITH_PHDR) {
          ...
          *(TempPkt + MTP2_LINK_NUMBER_OFFSET) =
              ((header->rec.mc_hdlc.mc_header >> 16) & 0x01);
          *(TempPkt + MTP2_LINK_NUMBER_OFFSET + 1) =
              ((header->rec.mc_hdlc.mc_header >> 24) & 0xff);
      ...
      case ERF_TYPE_IPV4:
        if ((p->linktype != DLT_RAW) && (p->linktype != DLT_IPV4))
          continue;
        packet_len = ntohs(header->wlen);
        caplen = rlen - dag_record_size;
        /* Skip over extension headers */
      ...
      case ERF_TYPE_IPV6:
        if ((p->linktype != DLT_RAW) && (p->linktype != DLT_IPV6))
          continue;
        packet_len = ntohs(header->wlen);
        caplen = rlen - dag_record_size;
        /* Skip over extension headers */
      ...
      /* These types have no matching 'native' DLT, but can be used with DLT_ERF
       * above */
      case ERF_TYPE_MC_RAW:
      case ERF_TYPE_MC_RAW_CHANNEL:
    ...
    if ((p->fcode.bf_insns == nullptr) ||
      ...
      /* Only count packets that pass the filter, for consistency with standard
       * Linux behaviour. */
      processed++;
...
/*
 *  Get a handle for a live capture from the given DAG device.  Passing a
 * nullptr device will result in a failure.  The promisc flag is ignored because
 * DAG cards are always promiscuous.  The to_ms parameter is used in setting the
...
static int dag_activate(pcap_t *p) {
  ...
  if (device == nullptr) {
    snprintf(p->errbuf, PCAP_ERRBUF_SIZE, "device is nullptr");
    return PCAP_ERROR;
  }
  ...
  if (dag_parse_name(device, newDev, strlen(device) + 16, &pd->dag_stream) <
      0) {
    /*
  ...
  if ((pd->dag_ref = dag_config_init((char *)device)) == nullptr) {
    /*
  ...
  if (dag_component_get_subcomponent(pd->dag_root, kComponentStreamFeatures,
                                     0)) {
    pd->drop_attr = dag_config_get_indexed_attribute_uuid(
        pd->dag_ref, kUint32AttributeStreamDropCount, pd->dag_stream / 2);
  }

  /* Set up default poll parameters for stream
   * Can be overridden by pcap_set_nonblock()
   */
  if (dag_get_stream_poll64(p->fd, pd->dag_stream, &mindata, &maxwait, &poll) <
      0) {
    ret = PCAP_ERROR;
    pcap_fmt_errmsg_for_errno(p->errbuf, PCAP_ERRBUF_SIZE, errno,
                              "dag_get_stream_poll");
  ...
  if (dag_set_stream_poll64(p->fd, pd->dag_stream, mindata, &maxwait, &poll) <
      0) {
    ret = PCAP_ERROR;
    pcap_fmt_errmsg_for_errno(p->errbuf, PCAP_ERRBUF_SIZE, errno,
                              "dag_set_stream_poll");
  ...
  if ((0x4200 == daginf->device_code) || (0x4230 == daginf->device_code)) {
    /* DAG 4.2S and 4.23S already strip the FCS.  Stripping the final word again
    ...
    if ((s = getenv("ERF_FCS_BITS")) != nullptr) {
     ...
     */
    if ((s = getenv("ERF_DONT_STRIP_FCS")) != nullptr) {
...
static int dag_stats(pcap_t *p, struct pcap_stat *ps) {
   ...
   * Packet records dropped (ps_drop) are read from Stream Drop attribute if
   * present, otherwise integrate the ERF Header lctr counts (if available) in
   * dag_read(). We are reporting that no records are dropped by the card/driver
   * (ps_ifdrop).
   */

  if (pd->drop_attr != kNullAttributeUuid) {
    /* Note this counter is cleared at start of capture and will wrap at
     * UINT_MAX. The application is responsible for polling ps_drop frequently
     * enough
     * to detect each wrap and integrate total drop with a wider counter */
    if ((dag_error =
             dag_config_get_uint32_attribute_ex(pd->dag_ref, pd->drop_attr,
                                                &stream_drop) == kDagErrNone)) {
...
int dag_findalldevs(pcap_if_list_t *devlistp, char *errbuf) {
  ...
  for (c = 0; c < DAG_MAX_BOARDS; c++) {
    ...
    if ((dagfd = dag_open(dagname)) >= 0) {
       ...
       */
      if (add_dev(devlistp, name, 0, description, errbuf) == nullptr) {
        /*
         * Failure.
         */
        return (-1);
...
static int dag_get_datalink(pcap_t *p) {
  ...
  if (p->dlt_list == nullptr &&
      (p->dlt_list = malloc(255 * sizeof(*(p->dlt_list)))) == nullptr) {
