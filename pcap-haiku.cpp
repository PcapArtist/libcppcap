...
static int pcap_read_haiku(pcap_t *handle, int maxPackets,
                           pcap_handler callback, u_char *userdata) {
  ...
  if (bytesReceived < 0) {
    ...
    snprintf(handle->errbuf, sizeof(handle->errbuf), "recvfrom: %s",
             strerror(errno));
    return -1;
  }
...
extern "C" pcap_t *pcap_create_interface(const char *device,
                                         char *errorBuffer) {
  ...
  pcap_t *handle = pcap_create_common(errorBuffer, sizeof(struct pcap_haiku));
  if (handle == nullptr) {
...
static int can_be_bound(const char *name) { return 1; }
...
extern "C" int pcap_platform_finddevs(pcap_if_list_t *_allDevices,
                                      char *errorBuffer) {
