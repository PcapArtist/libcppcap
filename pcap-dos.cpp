 ...
 *
 *  pcap-dos.cpp: Interface to PKTDRVR, NDIS2 and 32-bit pmode
 *              network drivers.
 */
...
extern void dbug_write(const char *); /* Watt-32 lib, pcdbug.cpp */
...
static struct device ndis_dev = {
    "ndis",    "NDIS2 LanManager", 0, 0, 0, 0, 0, 0,
    0,         NDIS_NEXT_DEV, /* nullptr or a 32-bit device */
    ndis_probe};

static struct device pkt_dev = {"pkt", "Packet-Driver", 0,        0, 0, 0, 0, 0,
                                0,     &ndis_dev,       pkt_probe};
...
static int pcap_activate_dos(pcap_t *pcap) {
  ...
  pcap->setdirection_op = nullptr; /* Not implemented.*/
  pcap->fd = ++ref_count;
  ...
  if (pcap->fd == 1) /* first time we're called */
  {
    ...
    snprintf(pcap->errbuf, PCAP_ERRBUF_SIZE,
             ...
             "(`%s' vs. `%s')",
             active_dev->name, pcap->opt.device);
    /* XXX - free pcap->buffer? */
    return (PCAP_ERROR);
  }
 ...
 * with the packet.
 */
static int pcap_read_one(pcap_t *p, pcap_handler callback, u_char *data) {
  ...
  while (!exc_occured) {
    ...
    if (rx_len > 0) /* got a packet */
    {
      ...
      if (callback &&
          (!p->fcode.bf_insns ||
           pcap_filter(p->fcode.bf_insns, p->buffer, pcap.len, pcap.caplen))) {
        filter_count++;

        /* Fix-me!! Should be time of arrival. Not time of
         * capture.
         */
        gettimeofday2(&pcap.ts, nullptr);
...
static int pcap_stats_dos(pcap_t *p, struct pcap_stat *ps) {
  ...
  struct pcap_dos *pd;
  struct device *dev = p ? get_device(p->fd) : nullptr;
  ...
  if (!dev->get_stats || (stats = (*dev->get_stats)(dev)) == nullptr) {
 ...
 */
u_long pcap_mac_packets(void) { return (mac_count); }

/*
 * Return # of packets passed through filter in pcap_read_dos()
 */
u_long pcap_filter_packets(void) { return (filter_count); }
...
static void pcap_cleanup_dos(pcap_t *p) {
  ...
  if (!exc_occured) {
    pd = p->priv;
    if (pcap_stats(p, nullptr) < 0)
      pd->stat.ps_drop = 0;
    if (!get_device(p->fd))
      return;

    handle_to_device[p->fd - 1] = nullptr;
    p->fd = 0;
    if (ref_count > 0)
      ref_count--;
    if (ref_count > 0)
...
int pcap_platform_finddevs(Interfaces *devlistp, char *errbuf) {
  ...
  for (dev = (struct device *)dev_base; dev; dev = dev->next) {
     ...
     */
    if ((curdev = add_dev(devlistp, dev->name, 0, dev->long_name, errbuf)) ==
        nullptr) {
      ret = -1;
      break;
    }
 ...
 */
static struct device *open_driver(const char *dev_name, char *ebuf,
                                  int promisc) {
  struct device *dev;
  ...
  for (dev = (struct device *)dev_base; dev; dev = dev->next) {
    ...
    if (!probed_dev) /* user didn't call pcap_lookupdev() first */
    {
      ...
      {
        snprintf(ebuf, PCAP_ERRBUF_SIZE, "failed to detect device `%s'",
                 dev_name);
        return (nullptr);
      }
    ...
    if (!(*dev->open)(dev)) {
      snprintf(ebuf, PCAP_ERRBUF_SIZE, "failed to activate device `%s'",
               dev_name);
...
static void init_32bit(void) {
  ...
  if (!_printk_file)
    _printk_init(64 * 1024, nullptr); /* calls atexit(printk_exit) */
...
static void *pcap_recv_hook(WORD *type) {
  int len = pcap_read_dos(&pcap_save, 1, watt32_recv_hook, nullptr);
 ...
 * using normal PKTDRVR functions in pcpkt.cpp. This should hopefully
...
static int init_watt32(struct pcap *pcap, const char *dev_name, char *err_buf) {
  ...
  MTU = pkt_get_mtu();
  pcap_save.fcode.bf_insns = nullptr;
  pcap_save.linktype = _eth_get_hwtype(nullptr, nullptr);
...
static const struct config_table debug_tab[] = {
    ...
    {"PKT.VECTOR", ARG_ATOX_W, nullptr},
    {"NDIS.DEBUG", ARG_ATOI, nullptr},
#ifdef USE_32BIT_DRIVERS
    ...
    {"3C505.DEBUG", ARG_ATOI, nullptr},
    {"3C505.BASE", ARG_ATOX_W, nullptr},
    {"3C507.DEBUG", ARG_ATOI, nullptr},
    {"3C509.DEBUG", ARG_ATOI, &el3_debug},
    {"3C509.ILOOP", ARG_ATOI, &el3_max_loop},
    {"3C529.DEBUG", ARG_ATOI, nullptr},
...
int pcap_config_hook(const char *keyword, const char *value) {
  return parse_config_table(debug_tab, nullptr, keyword, value);
}

/*
 * Linked list of supported devices
 */
struct device *active_dev = nullptr;      /* the device we have opened */
struct device *probed_dev = nullptr;      /* the device we have probed */
...
static int pkt_probe(struct device *dev) {
  ...
  dev->get_rx_buf = nullptr;
  dev->peek_rx_buf = nullptr;
  dev->release_rx_buf = nullptr;
...
static int ndis_probe(struct device *dev) {
  ...
  dev->copy_rx_buf = nullptr; /* to-do */
  dev->get_rx_buf = nullptr;  /* upcall is from rmode driver */
  dev->peek_rx_buf = nullptr;
  dev->release_rx_buf = nullptr;
  return (0);
}
...
struct device el2_dev LOCKED_VAR = {
    "3c503", "EtherLink II", 0, 0, 0, 0, 0, 0, 0, nullptr, el2_probe};

struct device el3_dev LOCKED_VAR = {
    "3c509", "EtherLink III", 0, 0, 0, 0, 0, 0, 0, &el2_dev, el3_probe};

struct device tc515_dev LOCKED_VAR = {
    "3c515", "EtherLink PCI", 0, 0, 0, 0, 0, 0, 0, &el3_dev, tc515_probe};

struct device tc59_dev LOCKED_VAR = {
    "3c59x", "EtherLink PCI", 0, 0, 0, 0, 0, 0, 0, &tc515_dev, tc59x_probe};

struct device tc90xbc_dev LOCKED_VAR = {
    "3c90x", "EtherLink 90X", 0, 0, 0, 0, 0, 0, 0, &tc59_dev, tc90xbc_probe};

struct device wd_dev LOCKED_VAR = {
    "wd", "Westen Digital", 0, 0, 0, 0, 0, 0, 0, &tc90xbc_dev, wd_probe};

struct device ne_dev LOCKED_VAR = {"ne", "NEx000", 0, 0,       0,       0,
                                   0,    0,        0, &wd_dev, ne_probe};

struct device acct_dev LOCKED_VAR = {
    "acct", "Accton EtherPocket", 0, 0, 0, 0, 0, 0, 0, &ne_dev, ethpk_probe};

struct device cs89_dev LOCKED_VAR = {
    "cs89",      "Crystal Semiconductor", 0, 0, 0, 0, 0, 0, 0, &acct_dev,
    cs89x0_probe};

struct device rtl8139_dev LOCKED_VAR = {
    "rtl8139", "RealTek PCI", 0, 0, 0, 0, 0, 0, 0,
    &cs89_dev, rtl8139_probe /* dev->probe routine */
};
 ...
 */
static void pktq_clear(struct rx_ringbuf *q) { q->in_index = q->out_index; }
